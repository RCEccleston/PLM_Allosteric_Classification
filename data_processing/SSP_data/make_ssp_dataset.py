...
import pandas as pd
from Bio import PDB
from Bio.PDB import PDBParser, NeighborSearch, DSSP
...
def extract_sequences_from_pdb(structure, pdb_file):
    dssp_path = "/home/lshre1/.local/bin/mkdssp"  # Manually set DSSP path

    NS_AA = {'MSE', 'CME', 'PTR', 'SEP', 'SNN'}  # Non-standard amino acids
    DSSP_MAP = {'H': 'H', 'G': 'H', 'I': 'H',  # Helix
                'E': 'E', 'B': 'E',             # Strand
                'T': 'C', 'S': 'C', '-': 'C'}  # Coil
    sequences = {}
    residue_numbers_by_chain = {}
    ...
    print(pdb_file)
    try:
        dssp = DSSP(model, pdb_file, dssp=dssp_path, file_type="PDB") 
        #dssp_dict = dict(dssp) 
    except Exception as e:
        print(f"Error processing DSSP for {pdb_file}: {e}")
        return None, None, None

    # Debug: Print available DSSP keys
    #print("DSSP Keys:", list(dssp_dict.keys())[:5])  # Print first few keys
    #for model in structure:
    for chain in model:
        sequence = ''
        residue_numbers = []
        ...
        for residue in chain:
            ...
            if residue_name.strip() != 'HOH':
                if PDB.is_aa(residue.get_resname(), standard=True):
                    ...
                    # Adjust residue key format for DSSP
                    dssp_key = (chain.get_id(), residue_id)  # Include insertion code
                    #print(dssp_key)
                    if dssp_key in dssp:
                        dssp_code = dssp[dssp_key][2]  # DSSP structure code
                        dssp_seq.append(DSSP_MAP.get(dssp_code, 'C'))  # Map to 3-class
                    else:
                        #print(f"Warning: DSSP key {dssp_key} not found!")
                        dssp_seq.append('C')  # Default to coil
                elif residue_name.strip() in NS_AA:
                    sequence += 'X'
                    residue_numbers.append(residue_id)
                    dssp_seq.append('C')

                # Match DSSP assignment (DSSP keys are tuples: (chain_id, residue_number))
                #dssp_key = (chain.get_id(), residue_id)
                #if dssp_key in dssp:
                #    ss_labels.append(dssp[dssp_key][2])  # Secondary structure column
                #else:
                #    ss_labels.append('C')  # Default to coil if DSSP entry is missing
        # Ensure DSSP labels match sequence length
        assert len(sequence) == len(dssp_seq), f"Mismatch in sequence and DSSP length for chain {chain.get_id()}"

        sequences[chain.get_id()] = sequence
        dssp_labels[chain.get_id()] = ''.join(dssp_seq)
        residue_numbers_by_chain[chain.get_id()] = residue_numbers

    return sequences, residue_numbers_by_chain, dssp_labels
...
def find_modulator_contacts(pdb_file, modulator_chains, modulator_residues, modulator_class, distance_threshold=7.0):
    ...
    sequences, res_numbers, secondary_structure = extract_sequences_from_pdb(structure, pdb_file)
    if not sequences:
    ...
    for chain in sorted(chains):
        ...
        full_ss +=secondary_structure[chain]

    return list(contact_residues), ''.join(full_sequence), full_labels, full_ss
...
def find_contacts(structure, modulator_chain, modulator_residue, threshold):
        ...
        if chain.id == modulator_chain:
            for res in chain.get_residues():
                if res.id[1] == modulator_residue:
                    modulator_atoms.extend(list(res.get_atoms()))
    contacts = ns.search_all(threshold, level='A')  
    contact_list = []
...
def generate_dssp(pdb_file):
    ...
    # Run DSSP
    model = structure[0]  # DSSP works on a single model
    dssp = DSSP(model, "example.pdb")

    # Extract secondary structure for each residue
    ss_labels = [dssp[key][2] for key in dssp.keys()]  # Column 2 is the secondary structure

    # Print results
    print(ss_labels)  # Example output: ['H', 'H', 'E', 'C', 'C', ...]
    return ss_labels
    ...
    for index, row in dataframe.iterrows():
        ...
        try:
            ...
            for chain, residues in zip(modulator_chains, modulator_residues):
                contacts, sequences, labels, secondary_structure = find_modulator_contacts(pdb_file, [chain], [residues], modulator_class, threshold)
                ...
                contact_data['Secondary_Structure'].append(secondary_structure)
...
# Directory paths
xml_directory = "/home/lshre1/Documents/PredictingAllostericSites/ASD_Release_202306_XF"
pdb_directory = '/home/lshre1/Documents/PredictingAllostericSites/ASD_Release_202306_XF_Allosteric_PDB'
...
allosteric_site_df.to_csv('ASD_Release_202306_XF_dataframe_7A_SSP.csv', index=False)
contacts_df.to_csv('ASD_Release_202306_XF_Allosteric_Contacts_7A_SSP.csv')
