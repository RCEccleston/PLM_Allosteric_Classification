...
"""
head = 'ConvBert'
train_file = '~/PLM_Allosteric_Classification/data/train_ortho.csv'
test_file = '~/PLM_Allosteric_Classification/data/test_ortho.csv'
...
train_orthosteric_labels = train_df['orthosteric_sites'].apply(ast.literal_eval).tolist()

# Split the dataframe (not lists) into validation and test sets
val_df, test_df = train_test_split(test_df_total, test_size=0.5, random_state=42, shuffle=True)

# Now extract sequences, labels, and orthosteric_labels for validation set
validation_sequences = val_df['Sequences'].tolist()
validation_labels = val_df['Labels'].apply(ast.literal_eval).tolist()
validation_orthosteric_labels = val_df['orthosteric_sites'].apply(ast.literal_eval).tolist()  # or whatever your column name is

# Same for test set
test_sequences = test_df['Sequences'].tolist()
test_labels = test_df['Labels'].apply(ast.literal_eval).tolist()
test_orthosteric_labels = test_df['orthosteric_sites'].apply(ast.literal_eval).tolist()


print(len(train_sequences), len(test_sequences), len(validation_sequences))
print(len(train_labels), len(test_labels), len(validation_labels))
print(len(train_orthosteric_labels), len(test_orthosteric_labels), len(validation_orthosteric_labels))
num_labels = 2
...
my_train = {"sequence" : train_sequences, "label" : train_labels, "mask" : train_masks, "orthosteric_sites" : train_orthosteric_labels}
my_train = pd.DataFrame(my_train)
my_valid = {"sequence" : validation_sequences, "label" : validation_labels, "mask" : validation_masks, "orthosteric_sites" : validation_orthosteric_labels}
my_valid = pd.DataFrame(my_valid)
my_test = {"sequence" : test_sequences, "label" : test_labels, "mask" : test_masks, "orthosteric_sites" : test_orthosteric_labels}
...
class T5EncoderForTokenClassification(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, class_config):
        ...
        # Create ortho layer
        self.ortho_embedding = nn.Linear(1, self.encoder.config.d_model)
    ...
    def forward(
        ...
        inputs_embeds=None,
        labels=None,
        orthosteric_sites=None,
        output_attentions=None,
        output_hidden_states=None,
        return_dict=None,
    ):
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # Create input embeddings if needed
        if inputs_embeds is None and input_ids is not None:
            inputs_embeds = self.encoder.embed_tokens(input_ids)
            input_ids = None

        # Inject orthosteric labels
        if orthosteric_sites is not None:
            # Project orthosteric mask to match embedding dim
            ortho_embeds = self.ortho_embedding(orthosteric_sites.unsqueeze(-1).float())
            # Add to input embeddings
            inputs_embeds = inputs_embeds + ortho_embeds
...
def load_T5_model_classification(
    checkpoint, filepath, num_labels, class_weights,
    half_precision, full=False, deepspeed=True, head='LogisticRegression',
    loss_type="CELoss"
):
    # 1. Load frozen encoder & tokenizer
    if "ankh" in checkpoint:
        ...
        model = T5EncoderModel.from_pretrained(checkpoint, torch_dtype=torch.float16 if half_precision and deepspeed else None)

    if getattr(model.config, "hidden_size", None) is None:
        model.config.hidden_size = getattr(model.config, "d_model", 1024)

    print(model.config.hidden_size)
    # 2. Wrap with classifier head
    ...
    # Use base encoder + shared weights
    ...
    # 3. Inject LoRA before loading weights
    peft_config = LoraConfig(
        r=8, lora_alpha=16, bias="none", target_modules=["q", "k", "v"],
        task_type=TaskType.TOKEN_CLS, inference_mode=False, lora_dropout=0.25
    )
    model = inject_adapter_in_model(peft_config, model)

    # 4. Load pretrained weights (LoRA + classifier)
    print(f"Loading LoRA/classifier weights from {filepath}")
    state_dict = torch.load(filepath, map_location="cpu")

    model_dict = model.state_dict() # full checkpoint model dict
    # filter out only those keys that exist in both dicts
    filtered_dict = {k: v for k, v in state_dict.items() if k in model_dict}
    # 5. Load weights from pretrained
    model_dict.update(filtered_dict)
    missing, unexpected = model.load_state_dict(model_dict, strict=False)
    print("Missing keys:", missing)
    print("Unexpected keys:", unexpected)

    # 6. Ensure classifier head is unfrozen
    ...
    # 7. Count trainable params
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"T5_LoRA_Classifier\nTrainable Parameters: {trainable_params}\n")
...
class EsmForTokenClassificationCustom(EsmPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # Create ortho layer
        self.ortho_embedding = nn.Linear(1, self.esm.config.hidden_size)
    ...
    def forward(
        ...
        orthosteric_sites: Optional[torch.LongTensor] = None,
        ...
        # Generate original embeddings from input_ids
        embedding_output = self.esm.embeddings(input_ids=input_ids, attention_mask = attention_mask)
        # Add orthosteric embeddings
        if orthosteric_sites is not None:
            # Project orthosteric mask to match embedding dim
            ortho_embeds = self.ortho_embedding(orthosteric_sites.unsqueeze(-1).float())
            # Add to input embeddings
            embedding_output = embedding_output + ortho_embeds


        # Convert attention_mask to bias form like ESM expects (same as BERT-like models internally do)
        if attention_mask is not None:
            # shape: (batch_size, 1, 1, seq_len)
            extended_attention_mask = attention_mask[:, None, None, :].to(dtype=embedding_output.dtype)  # float32 or float16
            extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0  # convert to additive bias mask
        else:
            extended_attention_mask = None
        

        outputs = self.esm.encoder(
            embedding_output,
...
class DataCollatorForTokenClassificationESM(DataCollatorMixin):
    ...
    def torch_call(self, features):
        ...
        sites_name_ortho = "orthosteric_sites"

        labels = [feature[label_name] for feature in features] if label_name in features[0].keys() else None
        sites_ortho = [feature[sites_name_ortho] for feature in features] if sites_name_ortho in features[0].keys() else None
        no_labels_features = [{k: v for k, v in feature.items() if k not in [label_name, sites_name_ortho]} for feature in features]
        ...
        def get_sequence_length(batch):
            ...
            raise ValueError("No valid tensor found for sequence length computation.")
        ...
        def pad_label_seq(label_seq):
            if padding_side == "right":
                return [
                    [self.label_pad_token_id] + to_list(label) +
        ...
        if labels is not None:
            batch[label_name] = torch.tensor(pad_label_seq(labels), dtype=torch.float)

        if sites_ortho is not None:
            # Treat orthosteric labels as **additional input**, not target
            batch[sites_name_ortho] = torch.tensor(pad_label_seq(sites_ortho), dtype=torch.float)
...
#load ESM2 models
def load_esm_model_classification(checkpoint, filepath, num_labels, class_weights, half_precision, full=False, deepspeed=True, head = 'LogisticRegression', loss_type = "CELoss"):
    ...
    # lora modification
    peft_config = LoraConfig(
        r=8, lora_alpha=16, bias="none", target_modules=["query","key","value"], task_type=TaskType.TOKEN_CLS, inference_mode=False, lora_dropout=0.25
    )
...
def create_dataset(tokenizer, seqs, labels, orthosteric_labels, checkpoint):
    tokenized = tokenizer(seqs, max_length=1024, padding="max_length", truncation=True)
    dataset = Dataset.from_dict(tokenized)

    # Automatically match the length of the tokenized input
    true_length = len(tokenized["input_ids"][0])  # should be 1024
    ...
    def pad_and_truncate(seq):
        seq = seq[:true_length]
        return seq + [-100] * (true_length - len(seq))  # use ignore index

    labels = [pad_and_truncate(l) for l in labels]
    orthosteric_labels = [pad_and_truncate(o) for o in orthosteric_labels]

    dataset = dataset.add_column("labels", labels)
    dataset = dataset.add_column("orthosteric_labels", orthosteric_labels)
...
def train_per_residue(
    ...
    filepath = "/home/lshre1/PredAllo/FT_New/best_model/esm_t36_secstr_finetuned.pth"
    ...
    if "esm" in checkpoint:
        model, tokenizer = load_esm_model_classification(checkpoint, filepath, num_labels, class_weights,  mixed, full, deepspeed, head, loss_type)
    else:
        model, tokenizer = load_T5_model_classification(checkpoint, filepath, num_labels, class_weights, mixed, full, deepspeed, head, loss_type)
    ...
    # Create Datasets
    train_set=create_dataset(tokenizer,list(train_df['sequence']),list(train_df['label']),list(train_df['orthosteric_sites']),checkpoint)
    valid_set=create_dataset(tokenizer,list(valid_df['sequence']),list(valid_df['label']),list(valid_df['orthosteric_sites']),checkpoint)
    test_set=create_dataset(tokenizer,list(test_df['sequence']),list(test_df['label']),list(test_df['orthosteric_sites']),checkpoint)


    # Huggingface Trainer arguments
    args = TrainingArguments(
        "./scripts/Finetuning/ESM2/",
    ...
    def compute_metrics(eval_pred):
        ...
        # If model returns a tuple (e.g., logits, extra_stuff), get logits only
        if isinstance(predictions, (tuple, list)):
            predictions = predictions[0]

        # Convert to tensors if not already
        predictions = torch.tensor(predictions)  # (batch, seq_len, num_classes)
        labels = torch.tensor(labels)            # (batch, seq_len)
        ...
        if predictions.shape[0] != labels.shape[0]:
            raise ValueError(f"Shape mismatch after masking: predictions={predictions.shape}, labels={labels.shape}")

        # Compute probabilities and predicted class
        probs = F.softmax(predictions.float(), dim=-1)
        ...
        # For binary tasks: get positive class probs
        pos_probs = probs_np[:, 1] if probs_np.shape[1] > 1 else probs_np[:, 0]
...
tokenizer, model, history = train_per_residue(checkpoint, my_train, my_valid, my_test, num_labels=2, batch=2, accum=8, epochs=10, seed=42, gpu=2, mixed = True, class_weights=class_weights, head = head, loss_type = "CELoss")
