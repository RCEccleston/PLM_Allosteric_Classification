...
flat_train_labels = [label for sublist in seg_train_labels for label in sublist]
...
weight_0, weight_1 = compute_weights(seg_train_labels)
...
my_train = {"sequence" : seg_train_sequences, "labels" : seg_train_labels, "mask" : train_masks}
my_train = pd.DataFrame(my_train)
my_valid = {"sequence" : seg_validation_sequences, "labels" : seg_validation_labels, "mask" : validation_masks}
my_valid = pd.DataFrame(my_valid)
my_test = {"sequence" : seg_test_sequences, "labels" : seg_test_labels, "mask" : test_masks}
...
save_model(model,"./PT5_secstr_finetuned_ankh_head.pth")
...
tokenizer, model_reload = load_model("./PT5_secstr_finetuned_ankh_head.pth", num_labels=2, mixed = True)
