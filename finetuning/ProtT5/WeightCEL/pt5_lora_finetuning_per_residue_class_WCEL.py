""" This script is adapted from https://github.com/agemagician/ProtTrans/blob/master/Fine-Tuning/PT5_LoRA_Finetuning_per_residue_class.ipynb """
""" Changes include adapting to a new dataset, adding the classification head ConvBertForMultiClassClassification from https://github.com/agemagician/Ankh and calculating additional metrics"""
...
def create_dataset(tokenizer,seqs,labels):
    ...
    input_lengths = [len(ids) for ids in tokenized["input_ids"]]

    labels = [l[:1023] for l in labels]
    dataset = dataset.add_column("labels", labels)
    print("Original sequence lengths:", [len(seq) for seq in seqs])
    print("Tokenized input lengths:", input_lengths)
    print("Labels lengths before truncation:", [len(l) for l in labels])
...
def train_per_residue(
    ...
    print("seq length 0= ", len(seqs[0]))
    labs = train_df["labels"]
    print("label length 0= ", len(labs[0]))
...
#    train_dataloader = DataLoader(my_train, batch_size=1, shuffle=False)
...
tokenizer, model, history = train_per_residue(my_train, my_valid, my_test, num_labels=2, batch=1, accum=1, epochs=30, seed=42, gpu=2, weight_0=weight_0, weight_1=weight_1)
