...
train_file = '/home/lshre1/PredAllo/train_df_pdb.csv'
test_file = '/home/lshre1/PredAllo/test_df_pdb.csv'
...
def compute_loss(model, inputs):
    ...
    #labels = inputs["labels"].long().to(logits.device)  # Convert labels to LongTensor and match device
    loss_fct = nn.CrossEntropyLoss(weight=class_weights)
    active_loss = inputs["attention_mask"].view(-1) == 1
    #active_logits = logits.view(-1, model.config.num_labels)
    active_logits = logits.logits.view(-1, 2)
    #active_labels = torch.where(
    ...
    valid_labels=active_labels[active_labels!=-100]
    valid_labels = valid_labels.type(torch.LongTensor).to('cuda:0')
    print("Logits dtype:", valid_logits.dtype)
    print("Labels dtype:", valid_labels.dtype)
    print("Logits device:", valid_logits.device)
    print("Labels device:", valid_labels.device)
...
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        outputs = model(**inputs)  # TokenClassifierOutput object
        logits = outputs.logits  # Extract logits tensor
        logits = logits.float()  # Ensure logits are float32
        labels = inputs["labels"].long().to(logits.device)  # Ensure labels are int64 and on the same device
...
def train_per_residue(
    ...
    # Create Datasets
    train_set=create_dataset(tokenizer,list(train_df['sequence']),list(train_df['labels']))
    valid_set=create_dataset(tokenizer,list(valid_df['sequence']),list(valid_df['labels']))
    test_set=create_dataset(tokenizer,list(test_df['sequence']),list(test_df['labels']))
    ...
    args = TrainingArguments(
        output_dir = "PT5_Perf_pdb_ankh_head_10/",
        eval_strategy = "epoch",
        eval_steps = 500,
    ...
    save_path = os.path.join("lora_binding_sites", f"best_model_pt5_lora_pdb_10")
...
tokenizer, model, history = train_per_residue(my_train, my_valid, my_test, num_labels=2, batch=1, accum=1, epochs=20, seed=42, gpu=2, weight_0=weight_0, weight_1=weight_1)
...
def save_model(model, filepath):
    ...
    # Create a dictionary to hold the non-frozen parameters' state_dict entries
    non_frozen_state_dict = {}

    # Iterate through all the model parameters
    for param_name, param in model.named_parameters():
        # If the parameter has requires_grad=True, add its value (not the tensor) to the dictionary
        if param.requires_grad:
            non_frozen_state_dict[param_name] = param.detach().cpu().clone()

    # Save only the finetuned parameters
    torch.save(non_frozen_state_dict, filepath)


def load_model(filepath, num_labels, weight_0, weight_1, mixed=True):
    # Creates a new PT5 model and loads the finetuned weights from a file

    # Load a new model
    model, tokenizer = PT5_classification_model(num_labels=num_labels, weight_0=weight_0, weight_1=weight_1, half_precision=mixed)
    # Load the non-frozen parameters (state dict) from the saved file
    non_frozen_state_dict = torch.load(filepath)

    # Assign the non-frozen parameters to the corresponding parameters of the model
    for param_name, param in model.named_parameters():
        if param_name in non_frozen_state_dict:
            param.data.copy_(non_frozen_state_dict[param_name])
...
save_model(model,"./PT5_secstr_finetuned_ankh_head_pdb_20.pth")
...
tokenizer, model_reload = load_model("./PT5_secstr_finetuned_ankh_head_pdb_20.pth", num_labels=2, mixed = True)
